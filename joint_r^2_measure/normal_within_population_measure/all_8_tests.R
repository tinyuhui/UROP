...
likelihood_test = function(p, q, size1, size2){
  ...
  exp[1] = (size1*p[1] + size2*q[1])/(size1 + size2)
  exp[2] = (size1*p[2] + size2*q[2])/(size1 + size2)
  exp[3] = (size1*p[3] + size2*q[3])/(size1 + size2)
  exp[4] = (size1*p[4] + size2*q[4])/(size1 + size2)
  
  tests = numeric(8)
  
  for (i in 1:4){
    tests[i] = (-1 + 2* pnorm(sqrt((size1 + size2) / (exp[i]*(1-exp[i])))*abs(exp[i] - p[i])))
    tests[i+4] = (-1 + 2* pnorm(sqrt((size1 + size2) / (exp[i]*(1-exp[i])))*abs(exp[i] - q[i])))
  }
...
create_pdf = function(p0, q0, N, M, c, migration, simulations, no_breaks, title = 'no title'){
  ...
  values_of_tests = matrix(0, nrow = simulations, ncol = 8)
  ...
  for (i in 1:8){
    hist(values_of_tests[,i], breaks = no_breaks, col = 'lightblue', xlab = 'r^2 simulated values', 
         main = paste(tit, 'test', as.character(i), sep = " "))
  }
